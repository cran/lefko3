...
```
  ...
  <p style="text-indent: 20px">A close look at the resulting object, `cypframe`, shows a data frame that includes in order for each stage: the stage's name, the associated size, its reproductive status, its status as an observable stage, its status as a propagule stage, its status as an immature stage, its status as a mature stage, whether it occurs in the dataset, the half-width of its size class bin, the minimum and maximum ages associated with the stage, the minimum and maximum of its size class bin, the centroid of its size class bin, its full size class bin width, and a comments field describing the stage. Stage names and combinations of characteristics must be unique to prevent estimation errors, and the comments field may be edited to include any information deemed pertinent.</p>
...
```
  ...
  <p style="text-indent: 20px">This reorganized dataset includes a set of interesting terms, the `sizeadded` group of three variables. These are sums of the size variables for each time, such that `size1added` is calculated as `sizea1 + sizeb1 + sizec1`. This may or may not make sense depending on the dataset. In this dataset, the full size of the individual in each time is this sum, because size is determined as the number of stems per plant, and the 3 original size columns give the numbers of 3 different kinds of stems: `Veg`, `Inf`, and `Inf2` are thes number of non-reproductive stems, single-flowered inflorescences, and double-flowered inflorescences, respectively, per plant per time-step (an inflorescence is a single stem, and no inflorescence has more than two flowers). Since size is given by the total stems in this example, we will use the `sizeadded` group of variables to code individual size.</p>
  
  <p style="text-indent: 20px">Size and fecundity are both count variables here, and we need to determine the appropriate distribution to use in both cases. Let's look at a histogram of size in time *t*.</p>

```r
hist(vertdata$size2added, main = "Size in time t", xlab = "Number of stems in time t")
```

![Figure 5.2. Histogram of Cypripedium candidum size in time t](Ch5.3-1.png)

  <p style="text-indent: 20px">Stem number has a very low mean, and the curve declines fairly smoothly. The Poisson or negative binomial distribution might work for size. Before determining whether this is indeed the case, let's also assess fecundity. As we see below, fecundity appears to have many 0s, suggesting that we might need a zero-inflated distribution.</p>
...
```
  ...
  <p style="text-indent: 20px">This dataset is longer and narrower, with more rows and fewer columns. This is because we split data for each individual across multiple rows. After three columns of identifying information (`plantid`, `patch`, and `censor`), a single column designates time in year *t*, given as `year2`. This dataset then includes columns showing individual state in pairs of consecutive years corresponding to times *t* and *t*+1. State in time *t*-1 is not presented because this is an ahistorical dataset. This dataset includes the `plantid` variable, which is an individual identity term that allows us to associate rows with their individuals and so allows conversion. The `historicalize3()` function uses individual identity to reorganize datasets into historical vertical format.</p>
...
cypsupp3 <- supplemental(stage3 = c("SD", "SD", "P1", "P1", "P2", "P3", "SL",
  ...
  type = c("S", "S", "S", "S", "S", "S", "S", "S", "S", "S", "S", "S", "S", "S",
    "S", "S", "S", "R", "R"), 
  type_t12 = c("S", "F", "S", "F", "S", "S", "S", "S", "S", "S", "S", "S", "S", "S",
    "S", "S", "S", "S", "S"), stageframe = cypframe)
...
#### Step 3c. Model building and selection
  ...
  8) juvenile size in time *t*+1 assuming survival and observation in that time, and
  ...
  <p style="text-indent: 20px">At the time of writing, mixed modeling of the negative binomial distribution, as well as of zero-truncated and zero-inflated versions of the Poisson and negative binomial distributions, have been deactivated due to core issues with package `glmmTMB`. So, here we utilize a mixed approach but with a standard Poisson response for size and fecundity. We will update this section again once mixed models with these response distributions are enabled again.</p>
...
cypmodels3 <- modelsearch(vertdata, historical = TRUE, approach = "mixed", 
  vitalrates = c("surv", "obs", "size", "repst", "fec"), sizedist = "poisson", 
  size.trunc = FALSE, fecdist = "poisson", fec.zero = FALSE, suite = "full", 
...
#> Warning in checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv, : Model failed to converge with max|grad| =
#> 0.016453 (tol = 0.002, component 1)
...
```
  ...
  <p style="text-indent: 20px">Once again, we see many more total elements (over 8.5 million, in comparison to 2,916 in the ahistorical case), and many more rows and columns (54 rows and columns in the ahistorical case, and 54<sup>2</sup> = 2,916 rows and columns in the historical case). However, historical matrices are composed primarily of structural 0s. Thus, with only 120,064 elements estimated per matrix, only 1.4% of elements are non-zero (the equivalent percentage for the ahistorical case is 84.3%).</p>

  <p style="text-indent: 20px">Now let's estimate some element-wise mean matrices. First, we will create the ahistorical mean matrix. We will calculate the column sums, which represent the survival probabilities of stages from time *t* to time *t*+1.</p>

```r
tmeans2r <- lmean(cypmatrix2)
summary(colSums(tmeans2r$U[[1]]))
#>    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#>  0.1000  0.9538  0.9866  0.9068  0.9967  0.9994
```

  <p style="text-indent: 20px">Column sums are very useful tools in checking yourself, because they are equal to the overall annual survival of the stage or stage pair, in ahMPMs and hMPMs, respectively. The use of externally-supplied rates and probabilities, as supplied via the supplement and overwrite tables, can result in some stages or stage-pairs having survival probabilities greater than 1.0, and if that occurs, then the supplement/overwrite tables need to be corrected to reduce survival to something reasonable.</p>
  
  <p style="text-indent: 20px">Next we will estimate the historical element-wise mean matrix, along with its column sums (these represent the survival of stage pairs in time *t*-1 and time *t*).</p>

```r
tmeans3r <- lmean(cypmatrix3)
summary(colSums(tmeans3r$U[[1]]))
#>    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#>  0.0000  0.3988  0.9162  0.6928  0.9849  0.9996
```
...
```
  ...
  <p style="text-indent: 20px">For further comparison, let's decompose the mean historical matrix into conditional historical matrices. We'll take a look at the matrix conditional on vegetative dormancy in time *t*-1.</p>
...
#> 1   1     1 -0.07248194 0.02684375 0.1638406 0.001638406
cypann3s
#>   pop patch           a       var        sd          se
#> 1   1     1 -0.06464211 0.1386834 0.3724021 0.003724021
```
...
plot(lambda ~ year2, data = cypann2, xlab = "Year", ylab = "Lambda", ylim = c(0.8, 0.98), type = "l", lty= 1, lwd = 2, bty = "n")
...
![Figure 5.6. Ahistorical vs. historical annual population growth rate](Ch5.24-1.png)
  ...
  <p style="text-indent: 20px">We find some intriguing differences in annual $\lambda$ estimates differ only slightly between ahistorical and historical analyses.</p>
...
![Figure 5.7. Ahistorical vs. historically-corrected stable and long-run mean stage distribution](Ch5.25-1.png)
  ...
  <p style="text-indent: 20px">Overall, all analyses suggest that small non-reproductive adults take up the greatest share of the stable stage structure, with dormant seed and 1st-year protocorms coming next. However, historical analyses then show a strong importance of small adults, particularly reproductive adults. So, history makes a big difference here.</p>
...
![Figure 5.8. Ahistorical vs. historically-corrected deterministic and stochastic reproductive values](Ch5.26-1.png)
  ...
  <p style="text-indent: 20px">We see some big differences here. The historical case predicts a greater importance of small and medium-sized flowering adults, and a lower importance of all other stages than the ahistorical case. In the ahistorical analyses, the highest reproductive values are in the largest flowering adults. So, both history and temporal stochasticity have large effects here. Interesting results in need of further study!</p>
...
```
  ...
  <p style="text-indent: 20px">Here, we see both deterministic sensitivity analyses supporting the transition from 2-sprouted vegetative adult in times *t*-1 and *t* to 9-sprouted flowering adult in time *t*+1. The stochastic sensitivity analyses supported the transition from 3-sprouted flowering adult in times *t*-1 and *t* to 13-sprouted flowering adult in time *t*+1. The highest historically-corrected deterministic sensitivity value is associated with the transition from 8-sprouted non-flowering adult to 13-sprouted flowering adults, while the historically-corrected stochastic sensitivity value is associated with the transitions from 9-sprouted non-flowering adult to the 13-sprouted flowering adult stage.</p>
...
```
  ...
  <p style="text-indent: 20px">Ahistorical deterministic and stochastic analyses supported the transition from 2-sprouted to 3-sprouted non-flowering adults. Historical deterministic and stochastic analyses suggest that population growth rate is most elastic in response to stasis as an 8-sprouted non-flowering adult. The highest elasticity in historically-corrected matrices occurred in stasis in 13-sprouted flowering adults in the deterministic case, and in stasis in 14-sprouted flowering adults in the stochastic case.</p>
...
![Figure 5.9. Ahistorical vs. historically-corrected deterministic and stochastic elasticity to stage](Ch5.30-1.png)
  ...
  <p style="text-indent: 20px">Elasticity analyses suggest very strong differences between ahistorical and historical analyses here, with smaller adult stages being most important in ahistorical analyses and intermediate-sized stages in historical analyses.</p>
...
text(cex=1, x=seq(from = -0.5, to = 2.8*length(tm3elas_sums$hist$category), by = 2.85),
  y=-0.075, tm3elas_sums$hist$category, xpd=TRUE, srt=45)
legend("topright", c("det ahist", "det hist", "sto ahist", "sto hist"),
  col = c("orangered", "darkred"), pch = 15, bty = "n")
```

![Figure 5.11. Elasticity of lambda to historical transitions](Ch5.32-1.png)
  <p style="text-indent: 20px">We can see that growth from time *t*-1 to *t* followed by shrinkage to time *t*+1, and the reverse, are associated with the greatest summed elasticities. Transitions associated with fecundity are associated with the lowest summed elasticities.</p>
