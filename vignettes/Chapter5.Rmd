...
#> grad| = 0.00275551 (tol = 0.002, component 1)
#> Warning in checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv, : Model failed to converge with max|
#> grad| = 0.0099624 (tol = 0.002, component 1)
#summary(cypmodels3)
```
...
#> Warning in checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv, : Model failed to converge with max|
#> grad| = 0.0330464 (tol = 0.002, component 1)
#summary(cypmodels2)
```
...
#>  0.1000  0.9910  0.9999  0.9249  1.0000  1.0000
```
...
#> 1   1     1 -0.04430777 0.002874544 0.05361477 0.0005361477
cypann3s
#>   pop patch           a         var         sd           se
...
```
  ...
  <p style="text-indent: 20px">Here, we see both deterministic and stochastic sensitivity analyses supporting the transition from 1-sprouted vegetative adult in times *t*-1 and *t* to the largest flowering adult in time *t*+1. The highest historically-corrected deterministic sensitivity value is associated with the transition from 1-sprouted non-flowering adult to 20-sprouted flowering adults, while the historically-corrected stochastic sensitivity value is associated with the transitions from 1-sprouted non-flowering adult to the largest flowering adult stage. Thus, our results generally reinforce the results of the ahistorical case.</p>
