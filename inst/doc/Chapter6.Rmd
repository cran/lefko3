...
### OVERALL GOALS AND INITIAL CONSIDERATIONS
  ...
  <p style="text-indent: 20px">First, we will clear memory and take a look at the dataset.</p>
...
![Figure 6.1. Histogram of fecundity in time t](Ch6.5-1.png)

  <p style="text-indent: 20px">We see that the distribution seems to conform to a classic count variable with a very low mean value. The first bar suggests that there may be too many zeroes. But to make that decision, let's formally test the two assumptions of $mean = variance$ and no excess 0s. Both tests use chi-squared distribution-based approaches, with the zero-inflation test based on van der Broek [-@van_den_broek_score_1995]. The following line will assess all of this in fecundity, but can also assess size if necessary (we will treat size as conforming to a Gaussian distribution, so will not test these assumptions with it).</p>
...
#### Step 3. Tests of history, and vital rate modeling
  ...
  **quiet**: Denotes whether to silence guidepost statements and most warning messages. Only warnings incurred in model testing will be visible, including warnings from the testing of global and reduced models produced during model dredging.
  ...
  <p style="text-indent: 20px">First, we will create the historical models to assess whether history is a significant influence on vital rates. Please remove the hashtag from the `summary` call to see details of the best-fit models.</p>
...
lathmodels3ipm <- modelsearch(lathvertipm, historical = TRUE, approach = "mixed", 
  ...
  fecdist = "poisson", fec.zero = FALSE, indiv = "individ", year = "year2", 
...
lathmodels2ipm <- modelsearch(lathvertipm, historical = FALSE, 
  ...
  fec.zero = FALSE, indiv = "individ", year = "year2", year.as.random = TRUE, 
  juvsize = TRUE, show.model.tables = TRUE, quiet = TRUE)
#> boundary (singular) fit: see ?isSingular
#> boundary (singular) fit: see ?isSingular
#> boundary (singular) fit: see ?isSingular
#> Warning in checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv, : Model failed to converge with max|grad| =
#> 0.10384 (tol = 0.002, component 1)
#> Warning in checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv, : Model is nearly unidentifiable: very large eigenvalue
#>  - Rescale variables?;Model is nearly unidentifiable: large eigenvalue ratio
#>  - Rescale variables?
#> boundary (singular) fit: see ?isSingular
...
```
  ...
  <p style="text-indent: 20px">These are giant matrices. With 10,609 rows and columns, there are a total of 112,550,881 elements per matrix. But they are also amazingly sparse - with 1,060,912 elements estimated, only 0.9% of elements per matrix are non-zero. Let's now build the ahistorical IPMs.</p>
...
```
  ...
  <p style="text-indent: 20px">Let's take a look at the column sums of the first ahistorical `U` matrix and the first historical `U` matrix, to make sure that all column sums look like survival probabilities.</p>
...
```
  ...
  <p style="text-indent: 20px">Everything looks OK, with stage survival probabilities all falling between 0 and 1.</p>
...
![Figure 6.2. Image of ahistorical projection matrix](Ch6.13-1.png)
...
![Figure 6.3. Image of historical projection matrix](Ch6.14-1.png)
...
![Figure 6.4. Ahistorical vs. historical lambda](Ch6.17-1.png)
...
#> 1   1     1 -0.2112523 0.005695228 0.07546673 0.0007546673
slambda3(lathmat3ipm, times = 1000)
#>   pop patch          a         var         sd          se
#> 1   1     1 -0.1501336 0.006138368 0.07834774 0.002477573
```
...
![Figure 6.5. Ahistorical vs. historically-corrected stable stage distribution](Ch6.19-1.png)
...
```
  ...
  <p style="text-indent: 20px">We will now move on to elasticity analysis. We will reduce the number of time steps simulated in the stochastic historical analysis to reduce the amount of computer processing time (this might take several hours even with the number of steps reduced). We will also remove the sensitivity analyses from memory, because these sensitivity and elasticity matrices are quite large.</p>
...
![Figure 6.7. Ahistorical vs. historically-corrected elasticity of lambda to stage](Ch6.23-1.png)
...
![Figure 6.8. Ahistorical vs. historically-corrected elasticity of lambda to transitions](Ch6.24-1.png)

  <p style="text-indent: 20px">We see similar transition elasticities between ahistorical and historical analyses, with growth and shrinkage the most influential on $\lambda$, while fecundity is by far the least influential.</p>
